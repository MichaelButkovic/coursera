...
object Huffman {
   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] = {
      if (chars.length == 1) return List((chars.head, 1))
      val sorted = chars.sorted
      sorted match {
        case Nil => Nil
        case x :: _ => {
          val (first, rest) = sorted span (y => y == x)
          List((first.head, first.length)) ++ times(rest)
        }
      }
    }
    ...
    def combine(trees: List[CodeTree]): List[CodeTree] = {
      if (trees.length < 2) trees
        ...
        trees match {
          case Nil => Nil
          case _ :: Nil => Nil
          case x :: x2 :: xs => makeCodeTree(x,x2) :: xs
        }
      }
    }
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def encodeHelper(t: CodeTree)(c: Char): List[Bit] = {
        t match {
          case Leaf(_, _) => Nil
          case Fork(left, right, _, _) => {
            if (chars(left).contains(c)) 0 :: encodeHelper(left)(c)
            else 1 :: encodeHelper(right)(c)
          }
        }
      }
