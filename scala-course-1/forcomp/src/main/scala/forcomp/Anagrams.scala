...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    if (occurrences.isEmpty) List(List())
      ...
      val (curr, count) = occurrences.head
      (for {
        i <- 0 to count
        item <- rest
      } yield if (i == 0) item else (curr, i) :: item).toList
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    y.toMap.foldLeft(x.toMap)((map: Map[Char, Int], tup: (Char, Int)) => {
      map.updated(tup._1, map(tup._1) - tup._2)
    }).filter(v => v._2 > 0).toList.sorted
  }
    ...
    def anagramHelper(occ: Occurrences): List[Sentence] = {
      if (occ.isEmpty) List(List())
        ...
        for {
          ...
          ana <- anagramHelper(subtract(occ, comb))
        } yield word :: ana
      }
    }
